...
import { useReadContract, useWriteContract, useSwitchChain, useAccount } from "wagmi";
...
const ZK_COMMUNITY_ADDRESS = process.env.NEXT_PUBLIC_ZK_COMMUNITY_ADDRESS as `0x${string}`;
...
export default function Home() {
  ...
  const renderContent = () => {
    if (!account.address) {
      return (
        <button onClick={() => open()} className="p-4 bg-blue-500 text-white rounded-full">
          Connect Wallet
        </button>
      );
    }
